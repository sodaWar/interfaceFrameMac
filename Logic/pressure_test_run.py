...
class PressureTest:
    ...
    def run_deal_one(self, result_id, test_case_list):
        ...
        for x in range(len(test_case_list)):                # 循环整个测试用例
            ...
            for i in range(pressure_num):
                t = threading.Thread(target=PressureTest.run_deal_two, args=(self, test_case_list[x],
    ...
    # 单个接口请求
    def run_deal_two(self, one_test_case, request_data_last, result_id):

        md = self.md
        conn = self.conn
        cur = self.cur

        ifd = PressureInterfaceDeal(one_test_case['num'], one_test_case['api_purpose'],
        ...
        if result_temp == 'success':
            ...
            """
            lock.acquire()                                      # 创建锁
            PressureInterfaceDeal.pressure_sql_two(md, conn, cur, 'success_num', result_id)
            lock.release()                                      # 释放锁
        elif result_temp == 'fail':
            lock.acquire()
            PressureInterfaceDeal.pressure_sql_two(md, conn, cur, 'fail_num', result_id)
            lock.release()
        elif result_temp == 'error':
            lock.acquire()
            PressureInterfaceDeal.pressure_sql_two(md, conn, cur, 'error_num', result_id)
            lock.release()
        else:
    ...
    def request_deal_two(request_data, pressure_test_file):
        ...
        special_key = PressureTest.request_deal_one(request_data)           # 请求数据中所有key值包含~的key值列表
        special_one = []
        ...
        for i in param:
            ...
            for x in range(len(param_two)):
                ...
                if x == len(param_two)-1:                                   # 一行数据赋值完毕,对下一行数据再次进行赋值,所以已赋值的数据存入列表内
                    ...
                    special_one = []
                    special_key = PressureTest.request_deal_one(request_data)
        ...
        for x in range(len(special_two)):                               # 相当于循环txt文件中的行数,如10行即代表要10个线程,请求10次接口
            for y in range(len(special_key)):                           # 相当于循环txt文件中的列数,作用是为了给每个请求数据中的相应key赋值
                key = special_key[y]
                request_data[key] = special_two[x][y][key]
